...
import { Flame, Star, Zap, Smile, Rocket, Ghost, Heart, Palette } from 'lucide-react';
...
export default function MoviesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/douban/movies');
  ...
  const getCategoryIcon = (name: string): React.JSX.Element => {
    ...
    const iconMap: Record<string, React.JSX.Element> = {
      '热门电影': <Flame className={`${iconClass} text-orange-500`} />,
      '豆瓣高分': <Star className={`${iconClass} text-yellow-500 fill-current`} />,
      '动作片': <Zap className={`${iconClass} text-red-500`} />,
      '喜剧片': <Smile className={`${iconClass} text-yellow-400`} />,
      '科幻片': <Rocket className={`${iconClass} text-blue-500`} />,
      '惊悚片': <Ghost className={`${iconClass} text-purple-500`} />,
      '爱情片': <Heart className={`${iconClass} text-pink-500 fill-current`} />,
      '动画电影': <Palette className={`${iconClass} text-green-500`} />,
    };
    return iconMap[name] || <Flame className={iconClass} />;
  };
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-md border-b border-gray-800/50">
        <div className="px-4 md:px-12 py-5">
          <div className="flex items-center justify-between">
            ...
            <h1 className="text-red-600 text-2xl md:text-3xl font-bold tracking-tight bg-gradient-to-r from-red-600 to-red-500 bg-clip-text text-transparent">
              壳儿
            </h1>
          </div>
        </div>
